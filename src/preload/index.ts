...
const copilotBridge = {
  ...
  initAgent: (config: { workspace: string; apiKey: string }) =>
    ipcRenderer.invoke('init-agent', config),
  ...
  createChat: (chat: { id: string; title: string; workspace: string }) =>
    ipcRenderer.invoke('create-chat', chat),
  addMessage: (msg: { id: string; chatId: string; sender: string; text: string }) =>
    ipcRenderer.invoke('add-message', msg),
  ...
  createScheduledTask: (task: {
    ...
    status: string
  }) => ipcRenderer.invoke('create-scheduled-task', task),
  updateScheduledTaskStatus: (update: { id: string; status: string }) =>
    ipcRenderer.invoke('update-scheduled-task-status', update),

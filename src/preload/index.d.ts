...
declare global {
  interface Window {
    ...
    copilot: {
      ...
      initAgent: (config: { workspace: string; apiKey: string }) => Promise<{ success: boolean; error?: string }>
      ...
      createChat: (chat: { id: string; title: string; workspace: string }) => Promise<{ success: boolean; error?: string }>
      addMessage: (msg: { id: string; chatId: string; sender: string; text: string }) => Promise<{ success: boolean; error?: string }>
      ...
      createScheduledTask: (task: { id: string; workspace: string; prompt: string; cronExpression: string; status: string }) => Promise<{ success: boolean; error?: string }>
      updateScheduledTaskStatus: (update: { id: string; status: string }) => Promise<{ success: boolean; error?: string }>
